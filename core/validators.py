...
def validate_fio(value, allow_empty_patronymic=True):
    """Валидация ФИО"""
    if not value or len(value.strip()) < 2:
        raise ValidationError('ФИО должно содержать минимум 2 символа')

    if len(value) > 50:
        raise ValidationError('ФИО не может быть длиннее 50 символов')
    ...
    if re.search(r'[!@#$%^&*()_+=\[\]{};:"\\|,.<>/?`~]', value):
        raise ValidationError('ФИО не может содержать специальные символы')

    # Проверяем на цифры
    if re.search(r'\d', value):
        raise ValidationError('ФИО не может содержать цифры')

    # Проверяем количество слов (минимум 2 - имя и фамилия)
    words = value.split()
    if len(words) < 2:
    ...
    for word in words:
        if len(word) < 2:
            raise ValidationError('Каждая часть ФИО должна содержать минимум 2 буквы')
        if not word[0].isupper() and not word[0] in 'АБВГДЕЁЖЗИЙЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ':
